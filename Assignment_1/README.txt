...
Our malloc first aligns the size to a multiple of 8 and adds 8 to count for the struct header size. It also accounts for the edge case when size = 0 which then it will add 8 more to the 
size. After this step, it traverses the heap array to find an open slot, and once we do, we allocate the item inside the heap array and reallocating and resizing the smaller chunk to fit 
its new destination. If the chunk is not able to fit inside of the heap, malloc will print an error and return NULL.
